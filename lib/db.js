...
export async function getAllGalleryImages() {
  try {
    const result = await sql`SELECT * FROM gallery_images ORDER BY display_order ASC, uploaded_at DESC`;
    return result.rows;
  } catch (error) {
...
export async function updateGalleryImageOrder(imageOrders) {
  try {
    for (const { id, display_order } of imageOrders) {
      await sql`UPDATE gallery_images SET display_order = ${display_order} WHERE id = ${id}`;
    }
  } catch (error) {
