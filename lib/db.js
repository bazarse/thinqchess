...
export async function updateAdminSettings(settings) {
  try {
    ...
    const result = db.prepare(`
      ...
      SET tournament_fee = ?, registration_fee = ?, max_participants = ?, countdown_end_date = ?, updated_at = ?
      WHERE id = (SELECT id FROM admin_settings ORDER BY id DESC LIMIT 1)
    `).run(tournament_fee, registration_fee, max_participants, countdown_end_date, new Date().toISOString());

    // Get the updated record
    const updated = db.prepare('SELECT * FROM admin_settings ORDER BY id DESC LIMIT 1').get();
    return updated;
  } catch (error) {
...
export async function validateDiscountCode(code) {
  try {
    const result = db.prepare(`
      ...
      WHERE code = ? AND is_active = 1 AND used_count < usage_limit
    `).get(code);
    return result;
  } catch (error) {
...
export async function getAllDiscountCodes() {
  try {
    const result = db.prepare(`SELECT * FROM discount_codes ORDER BY created_at DESC`).all();
    return result;
  } catch (error) {
