...
class SimpleDatabase {
  constructor(dbPath = './database.db') {
    ...
    this.dbPath = dbPath;
    this.dataPath = './data.json'; // Use current directory for local development
  ...
  loadData() {
    ...
    try {
      if (fs.existsSync(this.dataPath)) {
        ...
        console.log('📂 Loading existing data from file system');
        global.simpleDbData = parsedData;
        return parsedData;
      }
    } catch (error) {
      console.log('⚠️ Could not load from file system:', error.message);
    }

    console.log('🆕 Creating new database with default structure');
    const defaultData = {
      admin_settings: [],
      tournaments: [],
  ...
  saveData() {
    ...
    try {
      ...
      console.log('💾 Data saved to both memory and file system - Tables:', Object.keys(this.data).map(key => `${key}(${this.data[key].length})`).join(', '));
    } catch (error) {
      console.log('💾 Data saved to memory only - Tables:', Object.keys(this.data).map(key => `${key}(${this.data[key].length})`).join(', '));
    }
  }
