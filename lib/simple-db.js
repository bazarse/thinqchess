...
class SimpleDatabase {
  ...
  parseSetClause(setClause, params) {
    ...
    // Handle common SET patterns as fallback
    if (setClause.includes('is_active = ?') && !fields.is_active) {
      fields.is_active = params[paramIndex++];
    }
    if (setClause.includes('status = ?') && !fields.status) {
      fields.status = params[paramIndex++];
    }
    if (setClause.includes('demo_completed = ?') && !fields.demo_completed) {
      fields.demo_completed = params[paramIndex++];
    }
    if (setClause.includes('updated_at = ?') && !fields.updated_at) {
