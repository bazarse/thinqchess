...
function addSampleData() {
  try {
    // Add sample tournament if none exist
    const tournamentCheck = db.prepare('SELECT COUNT(*) as count FROM tournaments').get();
    if (!tournamentCheck || tournamentCheck.count === 0) {
      db.prepare(`
        INSERT INTO tournaments (title, description, start_date, end_date, fee, max_participants, is_active, status)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?)
      `).run(
        'Sample Chess Tournament',
        'A sample tournament for testing purposes',
        '2024-12-25',
        '2024-12-26', 
        500,
        50,
