 ...
 */
export async function updateTournamentStatus(db = null) {
  ...
  try {
    ...
    const completedResult = await sql`
      ...
      WHERE end_date < ${today} AND status != 'completed'
    `;

    results.completedCount = completedResult.rowCount || 0;
  ...
  try {
    ...
    const activeResult = await sql`
      ...
      WHERE start_date <= ${today} AND end_date >= ${today} AND status = 'upcoming'
    `;

    results.activeCount = activeResult.rowCount || 0;
...
export async function getTournamentsWithRegistrations(db = null, status = null) {
  try {
    ...
    if (status) {
      tournaments = await sql`SELECT * FROM tournaments WHERE status = ${status} ORDER BY end_date DESC`;
    } else {
      tournaments = await sql`SELECT * FROM tournaments ORDER BY end_date DESC`;
    }
    ...
    const tournamentsWithRegistrations = await Promise.all(tournaments.map(async tournament => {
      ...
      try {
        ...
        const regResult = await sql`
          ...
          WHERE tournament_id = ${tournament.id} AND payment_status = 'completed'
      ...
      if (registrations.length === 0) {
        try {
          const tournRegResult = await sql`
