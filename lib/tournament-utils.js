...
export async function updateTournamentStatus(db) {
  ...
  try {
    // Move tournaments to completed if end_date has passed (regardless of current status)
    // This handles tournaments that were never set to 'active' but have ended
    const completedResult = await db.run(
      "UPDATE tournaments SET status = 'completed' WHERE end_date < ? AND status != 'completed'",
      [today]
    );
