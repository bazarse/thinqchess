 ...
 */
export function updateTournamentStatus(db) {
  ...
  try {
    ...
    results.completedCount = completedResult.changes;
    
    if (completedResult.changes > 0) {
      console.log(`✅ Auto-updated ${completedResult.changes} tournaments to completed status`);
    }
  } catch (error) {
  ...
  try {
    ...
    results.activeCount = activeResult.changes;
    
    if (activeResult.changes > 0) {
      console.log(`✅ Auto-updated ${activeResult.changes} tournaments to active status`);
    }
  } catch (error) {
 ...
 */
export function getTournamentsWithRegistrations(db, status = null) {
  try {
    ...
    const tournamentsWithRegistrations = tournaments.map(tournament => {
      ...
      try {
        ...
        registrations = db.prepare(`
          ...
          WHERE tournament_id = ? AND payment_status = 'completed'
 ...
 */
export function getCompletedTournaments(db) {
  return getTournamentsWithRegistrations(db, 'completed');
}
 ...
 */
export function getActiveTournaments(db) {
  return getTournamentsWithRegistrations(db, 'active');
}
 ...
 */
export function getUpcomingTournaments(db) {
  return getTournamentsWithRegistrations(db, 'upcoming');
}
