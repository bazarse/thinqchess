...
let UPDATE_INTERVAL = 5 * 60 * 1000; // 5 minutes in milliseconds
 ...
 */
export async function autoUpdateTournamentStatus(db = null) {
  if (!shouldUpdate()) {
    return null;
  }
 ...
 */
export async function forceUpdateTournamentStatus(db = null) {
  try {
    const results = await updateTournamentStatus(db);
    lastUpdateTime = Date.now();
 ...
 */
export async function tournamentStatusMiddleware(db = null, force = false) {
  if (force) {
    return await forceUpdateTournamentStatus(db);
  } else {
    return await autoUpdateTournamentStatus(db);
  }
}
